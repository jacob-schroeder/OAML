...
namespace OAML.Security.Encryption
{
    public class RSAManager : IEncryptionManager
    {
        public EncryptionMethod _CryptMethod => EncryptionMethod.ASYMMETRIC;
        public int _KeySize => 2048;
        public KeyPair _Keys { get; private set; }

        public byte[] Decrypt(string pem, byte[] data)
        {
            try
            {
                using (var rsa = ImportPrivateKey(pem))
                {
        ...
        public byte[] Encrypt(string pem, byte[] data)
        {
            try
            {
                using (var rsa = ImportPublicKey(pem))
                {
        ...
        public KeyPair GenerateKeys()
        {
            using (var rsa = new RSACryptoServiceProvider(_KeySize))
            {
                try
                {
                    return KeyPair.Create(Encoding.UTF8.GetBytes(ExportPublicKey(rsa)), Encoding.UTF8.GetBytes(ExportPrivateKey(rsa)));
                }
                finally
                {
